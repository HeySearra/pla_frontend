<template>
  <div class="news">
    <div class="time_list">
      <div class="list_head">
        <h3 style="margin-right: 46px">辟谣专区</h3>
        <div>
          <el-radio-group v-model="type" size="small" @change="$router.replace({name: 'protect_tips'})">
      ...
      <div class="inner_time_list">
        ...
        <div v-if="loading" class="not_found" style="margin: 0 0 15px; padding-left: 25px"><i
          class="el-icon-loading"></i> 正在搜寻更多的辟谣知识
        </div>
        <div v-if="!loading&&!finished" @click="apply_for_info" class="not_found more"
             style="margin: 0 0 15px; padding-left: 25px">更多的辟谣知识
        </div>
        <div v-if="finished" class="not_found" style="margin: 0 0 15px; padding-left: 25px">辟谣知识就只有这么多了啦</div>
      </div>
    </div>
    <div class="inner_news">
      ...
      <div v-if="list" class="domestic">
        ...
        <!--            <Tip :id="`n${item.rid}`" :info="item" card_type="protect"/>-->
        <!--          </el-col>-->
        <!--        </el-row>-->
        <div class="masonry">
...
export default {
  ...
  mounted() {
    ...
    window.onresize = (function () {
      var a = function () {
        ...
        if (document.getElementById('app').clientWidth >= 1650) {
          that.span = 6;
        } else if (document.getElementById('app').clientWidth >= 1280) {
          that.span = 8;
        } else {
          that.span = 12;
        }
  ...
  data() {
    return {
      ...
      data_from: [{
          title: '辟谣信息（丁香园）',
          content: 'https://ncov.dxy.cn/ncovh5/view/pneumonia_rumors?target=5&from=dxy'
      }]
    }
  },
  ...
  methods: {
    ...
    apply_for_info() {
      ...
      this.ajax.post('/rumor/list', {}, this, {
        success(res) {
